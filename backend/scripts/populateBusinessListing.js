...
async function populateBusinessListing() {
  try {
    // Connect to MongoDB
    ...
    // Read the JSON file
    const jsonPath = path.join(__dirname, '../../business_listing_classification.json');
    const directoriesData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
    
    console.log(`📊 Found ${directoriesData.length} Business Listing entries`);
    ...
    // Clear existing Business Listing entries
    await Directory.deleteMany({ classification: 'Business Listing' });
    console.log('🗑️ Cleared existing Business Listing entries');
    ...
    // Verify insertion
    const count = await Directory.countDocuments({ classification: 'Business Listing' });
    console.log(`✅ Successfully inserted ${count} Business Listing entries`);

    // Show sample entries
    const sampleEntries = await Directory.find({ classification: 'Business Listing' }).limit(5);
    console.log('\n📋 Sample Business Listing entries:');
    sampleEntries.forEach((entry, index) => {
      console.log(`${index + 1}. ${entry.name} - ${entry.country}`);
    });

    // Show country breakdown
    const countryStats = await Directory.aggregate([
      { $match: { classification: 'Business Listing' } },
    ...
    countryStats.forEach(stat => {
      console.log(`   ${stat._id}: ${stat.count} platforms`);
    });
    ...
    console.error('❌ Error populating Business Listing:', error);
  } finally {
