...
async function populateBookMarking() {
  try {
    // Connect to MongoDB
    ...
    // Read the JSON file
    const jsonPath = path.join(__dirname, '../../bookmarking_classification.json');
    const directoriesData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
    
    console.log(`📊 Found ${directoriesData.length} BookMarking entries`);
    ...
    // Clear existing BookMarking entries
    await Directory.deleteMany({ classification: 'BookMarking' });
    console.log('🗑️ Cleared existing BookMarking entries');
    ...
    // Verify insertion
    const count = await Directory.countDocuments({ classification: 'BookMarking' });
    console.log(`✅ Successfully inserted ${count} BookMarking entries`);

    // Show sample entries
    const sampleEntries = await Directory.find({ classification: 'BookMarking' }).limit(5);
    console.log('\n📋 Sample BookMarking entries:');
    ...
    console.log(`\n🔖 High Priority bookmarking platforms: ${highPriorityCount}`);

  } catch (error) {
    console.error('❌ Error populating BookMarking:', error);
  } finally {
