...
async function populateProductionDB() {
  try {
    ...
    console.log('🧹 Clearing existing directories from production database...');
    ...
    for (const classification of Object.keys(classifications)) {
      ...
      console.log(`${classification}: ${count} directories in production database`);
    }
    
    console.log('\n🌐 Production database is now ready for use!');
