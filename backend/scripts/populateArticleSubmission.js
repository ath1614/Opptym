...
async function populateArticleSubmission() {
  try {
    // Connect to MongoDB
    ...
    // Read the JSON file
    const jsonPath = path.join(__dirname, '../../article_submission_classification.json');
    const directoriesData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
    
    console.log(`📊 Found ${directoriesData.length} Article Submission entries`);
    ...
    // Clear existing Article Submission entries
    await Directory.deleteMany({ classification: 'Article Submission' });
    console.log('🗑️ Cleared existing Article Submission entries');
    ...
    console.log(`📝 Inserted ${directoriesToInsert.length} Article Submission entries`);

    // Verify insertion
    const count = await Directory.countDocuments({ classification: 'Article Submission' });
    console.log(`✅ Successfully inserted ${count} Article Submission entries`);

    // Show sample entries
    const sampleEntries = await Directory.find({ classification: 'Article Submission' }).limit(5);
    console.log('\n📋 Sample Article Submission entries:');
    sampleEntries.forEach((entry, index) => {
      console.log(`${index + 1}. ${entry.name} - ${entry.domain}`);
    });

    // Show instant approval count
    const instantApprovalCount = await Directory.countDocuments({ 
      classification: 'Article Submission',
      priority: { $gte: 70 }
    });
    console.log(`\n⚡ Instant Approval platforms: ${instantApprovalCount}`);

  } catch (error) {
    console.error('❌ Error populating Article Submission:', error);
  } finally {
