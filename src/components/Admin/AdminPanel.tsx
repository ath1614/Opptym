...
export default function AdminPanel() {
  ...
  const handleUserFormSubmit = async () => {
    ...
    try {
      ...
      if (editingUser) {
        // Update user
        await axios.put(`/api/admin/users/${editingUser._id}`, userForm, { 
  ...
  const handleDirectoryFormSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`/api/admin/directories`, newDirectory, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
      setShowDirectoryModal(false);
      setNewDirectory({ name: '', domain: '', category: '', pageRank: 0, status: 'active' });
      // Refresh directories
      const res = await axios.get(`/api/admin/directories`, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
    ...
    try {
      ...
      if (action === 'suspend') {
        await Promise.all(selectedUsers.map(id =>
          axios.put(`/api/admin/users/${id}`, { status: 'suspended' }, { 
            headers: { Authorization: `Bearer ${token}` } 
          })
        ));
        ...
        await Promise.all(selectedUsers.map(id =>
          axios.put(`/api/admin/users/${id}`, { status: 'active' }, { 
            headers: { Authorization: `Bearer ${token}` } 
          })
        ));
      } else if (action === 'ban') {
        await Promise.all(selectedUsers.map(id =>
          axios.put(`/api/admin/users/${id}`, { status: 'banned' }, { 
            headers: { Authorization: `Bearer ${token}` } 
          })
        ));
      }
                    ...
                    <tr key={user._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="relative group">
                          ...
                          <div className="absolute right-0 top-8 bg-white dark:bg-primary-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg w-40 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                      ...
                      <tr key={project._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="relative group">
                            ...
                            <div className="absolute right-0 top-8 bg-white dark:bg-primary-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg w-32 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
